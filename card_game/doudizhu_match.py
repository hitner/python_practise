...
def _bin_cards_to_min3(cards):
    """
    移除花色，将'2'映射到15，并且进行排序
    :param cards:
    :return: 返回一个新的bytearray
    """
...
def check_all_bomb(bin_cards):
    """
    注意这里长度是没有检查的！
    :param cards:
    :return:
    """
    cards = _bin_cards_to_min3(bin_cards)
    if len(cards) == 2:
...
def check_valuable_play(bin_cards, cd=None) -> CardDescription:
    """
    :param cards: 通用bins牌
